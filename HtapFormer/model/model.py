...
from .htap_bias import HTAPBiasModule
...
class FeatureEmbed(nn.Module):
    def __init__(self, embed_size=32, tables = 10, types=20, joins = 40, columns= 30, \
        ...
        # type(embed_size) + filter(embed_size) + join(embed_size) + table(embed_size)
        self.project = nn.Linear(embed_size*4 + embed_size//8+1, embed_size*4 + embed_size//8+1)
    def forward(self, feature):

        typeId, joinId, filtersId, filtersMask, table = torch.split(feature, (1, 1, 9, 3, 1), dim = -1)
...
class HtapFormer(nn.Module):
    def __init__(self, emb_size = 32 ,ffn_dim = 32, head_size = 8, \
        ...
        self.use_htap_bias = use_htap_bias
        ...
        self.embbed_layer = FeatureEmbed(emb_size, joins=joins)
        ...
        if use_htap_bias:
            self.htap_bias_module = HTAPBiasModule(
                alpha_i=alpha_i,
                alpha_u=alpha_u,
                alpha_d=alpha_d,
                hidden_dim=ffn_dim,  
                ...
                query_weight_init=query_weight_init,
                operator_weight_init=operator_weight_init
            )
        else:
            self.htap_bias_module = None
    ...
    def forward(self, batched_data):
        ...
        # HTAP-Bias
        if self.use_htap_bias and self.htap_bias_module is not None and hasattr(batched_data, 'htap_info') and batched_data.htap_info is not None:
            htap_bias = self.htap_bias_module(batched_data.htap_info)
            htap_bias_expanded = htap_bias.unsqueeze(1).repeat(1, self.head_size, 1, 1)  # [batch, head_size, n_node, n_node]
            tree_attn_bias[:, :, 1:, 1:] = tree_attn_bias[:, :, 1:, 1:] + self.lambda_scale * htap_bias_expanded
