...
class Batch:
    def __init__(self, attn_bias, rel_pos, heights, x, y=None, htap_info=None):
...
def collator(small_set):
    ...
    htap_infos = []
    for s in small_set[0]:
        htap_info = s.get('htap_info', None)
        htap_infos.append(htap_info)
    
    return Batch(attn_bias, rel_pos, heights, x, htap_info=htap_infos), y
...
def formatJoin(json_node):
    ...
    if join is not None:
        alias = json_node.get('Alias', 'NA')
        twoCol = join[1:-1].split(' = ')
        twoCol = [alias + '.' + col if len(col.split('.')) == 1 else col for col in twoCol]
...
def map_node_type_to_operator(nodeType):

    nodeType_lower = nodeType.lower()
    
    
    if 'scan' in nodeType_lower:
        return 'Scan'
    
   
    if 'join' in nodeType_lower:
        return 'Join'
    
  
    if 'aggregate' in nodeType_lower:
        return 'Aggregate'
    
    if 'group' in nodeType_lower:
        return 'GroupBy'
...
class Encoding:
    ...
    def normalize_val(self, column, val, log=False):
        if column not in self.column_min_max_vals:
            if 'NA' in self.column_min_max_vals:
                mini, maxi = self.column_min_max_vals['NA']
            else:
                return 0.0
        else:
        ...
        for filt in filters:
            ...
            for f in fs:
                ...
                if len(parts) >= 3:
                    ...
                    col, op, num = '', '', ''
                alias = alias if alias is not None else 'NA'
                if col.startswith(alias + '.'):
                    ...
                    column = 'NA'
                res['colId'].append(self.col2idx.get(column, self.col2idx['NA']))
    ...
    def encode_join(self, join):
        return self.join2idx.get(join, 0)
...
class TreeNode:
    def __init__(self, nodeType, typeId, filt, card, join, join_str, filterDict):
        ...
        self.storage_mode = 'NA'        # 'row-store', 'column-store', 'NA'
        self.query_type = 'SELECT'      # 'INSERT', 'UPDATE', 'DELETE', 'SELECT'
        self.node_operator = 'Other'   # 'Scan', 'Join', 'Aggregate', 'GroupBy', 'Other'
        self.write_counts = {'insert': 0, 'update': 0, 'delete': 0}
        self.storage_bias_value = 0.0   # v_j^s 
        self.operator_bias_value = 0.0  # v_j^o 
