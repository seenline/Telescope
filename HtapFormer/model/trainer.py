...
def logging(args, epoch, qscores, filename = None, save_model = False, model = None):
    ...
    res = {}
    hashable_vals = []
    for key, val in zip(arg_keys, arg_vals):
        if isinstance(val, list):
            val_tuple = tuple(val)
            hashable_vals.append(val_tuple)
            res[key] = json.dumps(val)
        elif isinstance(val, tuple):
            hashable_vals.append(val)
            res[key] = json.dumps(list(val))
        else:
            hashable_vals.append(val)
            res[key] = val
    model_checkpoint = str(hash(tuple(hashable_vals))) + '.pt'
